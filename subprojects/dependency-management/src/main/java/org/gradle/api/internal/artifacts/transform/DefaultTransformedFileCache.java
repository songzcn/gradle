...
import org.gradle.internal.resource.local.CleanableSubDir;
...
public class DefaultTransformedFileCache implements TransformedFileCache, Stoppable, RootBuildLifecycleListener {
    ...
    public DefaultTransformedFileCache(ArtifactCacheMetadata artifactCacheMetadata, CacheRepository cacheRepository, InMemoryCacheDecoratorFactory cacheDecoratorFactory, FileSystemSnapshotter fileSystemSnapshotter) {
        ...
        CleanupAction cleanupAction = new SubDirCleanupAction(filesOutputDirectory,
            new FixedAgeOldestCacheCleanup(SECOND_LEVEL_CHILDREN, DEFAULT_MAX_AGE_IN_DAYS_FOR_RECREATABLE_CACHE_ENTRIES));
    ...
    private static class SubDirCleanupAction implements CleanupAction {
        private final File subDir;
        private final CleanupAction delegate;

        private SubDirCleanupAction(File subDir, CleanupAction delegate) {
            this.subDir = subDir;
        ...
        public void clean(CleanableStore cleanableStore) {
            delegate.clean(new CleanableSubDir(cleanableStore, subDir));
        }
    }
}
